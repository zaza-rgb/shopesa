    ...
    $ref_uti=5;
       ...
       if (isset($_POST['delete'])) {
          $id_commande=$_POST['id_commande'];
            $stmt = $com->prepare("DELETE FROM commande_produit WHERE id_commande=?");
            $stmt->execute([$id_commande]);
            $stmt = $com->prepare("DELETE FROM commande WHERE id_commande=?");
            $stmt->execute([$id_commande]);
...
$stmt = $com->prepare('
    SELECT c.id_commande, c.date_commande, c.statut,
           p.nomprod, cp.quantite, cp.prix_unitaire
    FROM commande c
    JOIN commande_produit cp ON c.id_commande = cp.id_commande
    JOIN produit p ON cp.idprod = p.idprod
    WHERE c.ref_uti = ?
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        <div class="item-card">
          <div class="item-info">
        <td>COMMANDE N°<?php echo $row['id_commande'] ?></td>
            <h3>ARTICLE: <?php echo htmlspecialchars($row['nomprod']); ?></h3>
            <h3>QUANTITE: <?php echo htmlspecialchars($row['quantite']); ?></h3>
            <h3>PRIX UNITAIRE: <?php echo htmlspecialchars($row['prix_unitaire']); ?></h3>
            <h3>STATUT: <?php echo htmlspecialchars($row['statut']); ?></h3>
            <p class="item-cat">DATE: <?php echo htmlspecialchars($row['date_commande']); ?></p>
            </div>
          </div>
          <?php
            ...
            <button type="submit" class="delete-link" name="delete" title="Supprimer">
