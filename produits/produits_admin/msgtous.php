...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['idtous']) && isset($_POST['mail'])) {
        try {
            $stmt = $com->query("SELECT email FROM utilisateur WHERE role = 'client'");
    ...
    elseif (isset($_POST['idtous']) && isset($_POST['notification'])) {
        try {
            $stmt = $com->query("SELECT ref_uti FROM utilisateur WHERE role = 'client'");
            $insert = $com->prepare("INSERT INTO notification (ref_uti, message,lu) VALUES (?, ?, ?)");
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $insert->execute([$row['ref_uti'], $description,'non_lu']);
            }
        } catch (PDOException $e) {
            echo "Erreur : " . $e->getMessage();
        }
      ...
      <div>
        ...
        <div class="card">
          ...
          <div class="field">
            ...
            <textarea name="description" placeholder="Ecrivez votre message..." required></textarea>
          </div>
        <hr class="divider" />
        <input type="hidden" name="idtous" value="<?php if(isset($_GET['idtous'])){echo $_GET['idtous'];} ?>">
